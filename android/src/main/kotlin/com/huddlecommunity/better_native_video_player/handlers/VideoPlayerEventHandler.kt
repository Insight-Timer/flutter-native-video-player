...
class VideoPlayerEventHandler(private val isSharedPlayer: Boolean = false) : EventChannel.StreamHandler {
    ...
    // One handler for all async event posts (previously allocated per event)
    ...
    override fun onListen(arguments: Any?, events: EventChannel.EventSink?) {
        NpLog.d(TAG, "onListen called - isSharedPlayer: $isSharedPlayer, hasCallback: ${initialStateCallback != null}")
        eventSink = events
        ...
        if (!isSharedPlayer) {
            NpLog.d(TAG, "Sending isInitialized event for new player")
            ...
            if (initialStateCallback != null) {
                NpLog.d(TAG, "Invoking initial state callback for new player")
            ...
            NpLog.d(TAG, "Invoking initial state callback for shared player")
            initialStateCallback?.invoke()
            hasSentInitialState = true
        }
    }
    ...
    fun sendEvent(name: String, data: Map<String, Any>? = null, synchronous: Boolean = false) {
        ...
        if (synchronous && eventSink != null) {
            ...
            NpLog.d(TAG, "Sending event synchronously: $name (eventSink is not null)")
            eventSink?.success(event)
        } else if (eventSink != null) {
            NpLog.d(TAG, "Sending event asynchronously: $name (eventSink is not null)")
            mainHandler.post {
                eventSink?.success(event)
            }
        } else {
            // No Flutter listener on this view - e.g. the floating player owns playback
            // on a shared view while Flutter is subscribed to a sibling view.
            val cid = controllerId
            if (cid != null &&
                SharedPlayerManager.routeEventToSubscribedView(cid, viewId, name, data)) {
                NpLog.d(TAG, "Routed event: $name to subscribed sibling view")
            } else {
                NpLog.w(TAG, "Cannot send event: $name - eventSink is null (EventChannel not ready yet)")
            }
        }
    }
}
