...
class PlayerBackendSession(
    ...
    init {
        ...
        // A shared player takes the cap of whichever backend shows it: an inline preview
        // caps itself, and the full-screen view that follows clears it again.
    ...
    /**
     * Current video size, falling back to the selected video track format while
     * `videoSize` is not populated yet.
     */
    fun resolveCurrentVideoDimensions(): Pair<Int, Int>? {
    ...
    fun emitCurrentState() {
        ...
        if (duration > 0) {
            ...
            if (videoWidth > 0 && videoHeight > 0) {
                payload["videoWidth"] = videoWidth
                payload["videoHeight"] = videoHeight
            }
